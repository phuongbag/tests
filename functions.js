...
const assignItem = (userData, reward) => {
    if (characters4Star.includes(reward)) {
        ...
        if (!userData.inventory.weapons.five_star[reward])
            ...
            userData.inventory.weapons.five_star[reward].rank++;
    }

    else if (weapons6Star.includes(reward)) {
        if (!userData.inventory.weapons.six_star)
            userData.inventory.weapons.six_star = {};
        if (!userData.inventory.weapons.six_star[reward])
            userData.inventory.weapons.six_star[reward] = [{...weaponInitialState}];
        else
        userData.inventory.weapons.six_star[reward].rank++;
    }
...
const printWeapons = (object, page) => {
    ...
    for(var prop in object.six_star) {
        if (object.six_star[prop].length > 0)
            curOutput += `- ${object.six_star[prop].length}x ${prop}\n`;
    }
    if (curOutput !== '') {
        output += '\`⭐⭐⭐⭐⭐⭐\`\n'
        output += '\`\`\`diff\n'
    ...
    if (curOutput !== '') {
        if (object.five_star[prop].rank > 5) {
            object.five_star[prop].rank = 5;
        }
        output += '\`⭐⭐⭐⭐⭐\`\n'
        output += '\`\`\`fix\n'
        output += curOutput;
...
const printCharacters = (object) => {
    ...
    curOutput = '';
    for(var prop in object.four_star) {
        if (object.four_star[prop].constellation_level > 6) {
            object.four_star[prop].constellation_level = 6;
        }
...
const calcStats = (baseAtk, baseHp, level, constellation, weaponDamage) => {
    ...
    var hp = Math.floor(baseHp * (1 + (level - 1)/7.5) * (constellation * 0.22 + 1));
    return [atk, hp];
};

const calcStatsWeapon = (baseAtk, level, rank) => {
    var atk = baseAtk * (1 + (rank *0.2 - 1)/10) * (level * 0.85 + 1);
    return atk;
};
