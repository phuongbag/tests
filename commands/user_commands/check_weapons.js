                ...
                if (constants.weapons.includes(msg_arr[1])) {
                    ...
                    userWeapons.forEach((weapon, index) => {
                        if (weapon.rank + weapon.level >= 3) {
                            msg += `\`\`\`${msg_arr[1]} #${index}: \n\nAtk: ${calcStatsWeapon(weaponConstants[msg_arr[1]].atk, weapon.level, weapon.rank)} \nLevel: ${weapon.level} \nRefine: ${weapon.rank} \nExp: ${weapon.stored_exp}/${weapon.level * 50} \n${weapon.isEquipped && weapon.isEquipped !== "" ? `User: ${weapon.isEquipped}` : ''}\n\`\`\``;
                            upgradedCount++;
                        }
                    });
