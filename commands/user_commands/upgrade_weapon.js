        ...
        try {
            if (msg_arr.length === 6 && weaponConstants[msg_arr[1]] !== undefined && weaponConstants[msg_arr[4]] !== undefined) {
                ...
                if (msg_arr[3].toUpperCase() === 'SINGLE'){
                    if (weapon && material) {
                        if (msg_arr[1] === msg_arr[4] && msg_arr[2] === msg_arr[5]){
                            message.channel.send(`<@${sender.id}> Bạn không thể sử dụng vũ khí hiện tại làm nguyên liệu.`);
                            return userData;
                        }
                        ...
                        if (weapon.level !== oldLevel)
                            msg += `\nWeapon level mới: ${oldLevel} -> ${weapon.level}`;
                        msg += `\nWeapon exp mới: ${weapon.stored_exp}/${weapon.level * levelUpExpMultiplier}`;
                        materials.splice(msg_arr[5], 1);
                        message.channel.send(`<@${sender.id}> đã cường hóa thành công ${msg_arr[1]} với ${msg_arr[4]}!${msg}`);
                    }
                }
                ...
                // WHEN UPGRADING USING MULTIPLE MATERIALS
                    ...
                    if (weapon) {
                        ...
                        if (newExp === weapon.stored_exp && oldLevel === weapon.level) {
                            message.channel.send(`<@${sender.id}> không có đủ ${msg_arr[4]}. Hãy dùng nguyên liệu khác.`);
                            return userData;
                        }
                        weapon.stored_exp = newExp;
                        if (weapon.level !== oldLevel)
                        ...
                        message.channel.send(`<@${sender.id}> Vũ khí không tồn tại.`);
                    }
                }
                else {
                    message.channel.send(`<@${sender.id}> Sai cú pháp`);
                }
                return userData;
            }
                ...
                if (weapon) {
                    ...
                    msg += `\nWeapon level mới: ${oldLevel} -> ${weapon.level}`;
                        msg += `\nWeapon exp mới: ${weapon.stored_exp}/${weapon.level * levelUpExpMultiplier}`;
                    message.channel.send(`<@${sender.id}> đã cường hóa thành công ${msg_arr[1]} với ${msg_arr[2]}!${msg}`);
                    return userData;
                }
                else {
                    message.channel.send(`<@${sender.id}> Vũ khí không tồn tại.`);
                ...
                message.channel.send(`<@${sender.id}> Sai cú pháp hoặc vũ khí không tồn tại.`);
                return userData;
            }
